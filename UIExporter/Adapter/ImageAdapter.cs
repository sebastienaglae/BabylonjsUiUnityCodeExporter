...
namespace PROJECT
{
	...
	public class ImageAdapter : ControlAdapter, IAdapter
	{
		...
		[Tooltip("Gets or sets a boolean indicating if pointers should only be validated on pixels with alpha > 0. Beware using this as this will comsume more memory as the image has to be stored twice")]
		public bool detectPointerOnOpaqueOnly;
		[Tooltip("Gets or sets a boolean indicating if nine patch slices (left, top, right, bottom) should be read from image data")]
		public bool populateNinePatchSlicesFromImage;
		[Tooltip("Gets the image source url")]
		...
		private IEnumerator GetTextureRequest(string url)
		{
			...
			using (var www = UnityWebRequestTexture.GetTexture(url))
			{
				...
				else
				{
					...
					imageComponent.sprite = Sprite.Create(texture, new Rect(Vector2.zero, new Vector2(texture.width, texture.height)), new Vector2(0.5f, 0.5f));
				}
			}
		}
	}
